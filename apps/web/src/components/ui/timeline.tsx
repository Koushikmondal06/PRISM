...
const defaultTopData: JourneyItem[] = [
  {
    id: "2024-q1",
    year: "2024",
    month: "Q1",
    content: "Solana LMSR bonding curve architecture initialized for continuous liquidity",
  },
  {
    id: "2024-q3",
    year: "2024",
    month: "Q3",
    content: "Polymarket truth discovery feed integrated for automated market sync",
  },
  {
    id: "2025-q2",
    year: "2025",
    month: "Q2",
    content: "Web-search backed AI oracle settlement engine deployed with 100% confidence QA",
  },
];

const defaultBottomData: JourneyItem[] = [
  {
    id: "2024-q2",
    year: "2024",
    month: "Q2",
    content: "Anchor smart contracts deployed on Solana testnet with USDC vault collateral",
  },
  {
    id: "2025-q1",
    year: "2025",
    month: "Q1",
    content: "LLM market curation pipeline goes live for automated title & tag optimization",
  },
  {
    id: "2026-q1",
    year: "2026",
    month: "Q1",
    content: "Full PRISM production release for high-speed real-world intelligence settlement",
  },
];
...
export default function Timeline({
  ...
  textColor = "#202329",
  mutedTextColor = "#6F747D",
  activeColor = "#10B981",
  backgroundColor = "transparent",
  imageUrl = "https://images.unsplash.com/photo-1639762681485-074b7f938ba0?auto=format&fit=crop&w=1000&q=80",
  imageAlt = "Decentralized Intelligence Network",
  topItems = defaultTopData,
  bottomItems = defaultBottomData,
  duration,
  scrollDuration = 1.2,
}: TimelineProps) {
  ...
  const allJourneyItems: JourneyItem[] = [...topItems, ...bottomItems];
  ...
  useGSAP(() => {
    ...
    items.forEach((item) => {
      try {
        ...
        // Fallback if SplitText is disabled
      }
    });
          ...
          <div className="relative h-full w-full">
            ...
            <div className="flex h-1/2 w-full items-center justify-start gap-[.5vw]">
              <div className="h-full w-[20%] pt-[2vw] max-[600px]:h-fit max-[600px]:pt-[5vw]">
                <h2 className="w-[85%] text-[2.8vw] font-extrabold leading-[0.95] max-[600px]:text-[8.5vw] color-[#202329]">
                  {title}
                </h2>
              </div>
                ...
                {topItems.map((item) => (
                  ...
                  >
                    ...
                    <div className="mt-[-1vw] space-y-[1vw] max-[600px]:mt-[-2vw]">
                      <h4
                        className={`title-${item.id} text-[2.2vw] font-bold leading-none max-[600px]:text-[6.4vw] color-[#202329]`}
                      >
                        {item.year} {item.month}
                      </h4>
                      <p
                        className={`description-${item.id} w-[90%] text-[1.3vw] leading-[1.25] max-[600px]:w-[90%] max-[600px]:text-[4.8vw]`}
            ...
            <div className="h-1/2 flex items-center justify-start w-full">
              <div className="w-[34%] pt-[2vw] max-[600px]:pt-[5vw] max-[600px]:w-[30%] h-full">
                <p
                  className="text-[1.5vw] font-semibold leading-none max-[600px]:text-[4.2vw]"
                  style={mutedTextStyle}
                >
                  {periodLabel}
                </p>
