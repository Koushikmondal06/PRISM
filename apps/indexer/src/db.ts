...
export async function storeMarket(market: StoredMarket): Promise<void> {
  ...
  try {
    ...
    try {
      await client.query(
        `INSERT INTO markets (polymarket_id, question, end_ts, price_yes_bps, lmsr_b, closed, winning_outcome, ai_score, ai_reason, ai_title, ai_tags, ai_summary, raw_data, source)
    ...
    console.warn('[indexer db] DB store skipped:', (err as Error).message);
  }
}
...
export async function updateMarketStatus(polymarketId: string, status: string, winningOutcome?: number, aiScore?: number): Promise<void> {
  ...
  try {
    ...
    console.warn('[indexer db] DB update skipped:', (err as Error).message);
  }
}
